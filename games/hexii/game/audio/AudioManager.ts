/**
 * Audio Manager for Hexii Game
 * Handles music playback with crossfading and SFX with throttling/pitch modulation
 */

export type MusicTrack = 'title' | 'gameplay' | 'boss' | 'defeat';
export type SFXType = 'turret-fire' | 'boss-appear' | 'boss-death' | 'player-hurt' | 'player-death' | 'enemy-death' | 'levelup' | 'ui-click' | 'ui-hover';
...
const MUSIC_PATHS: Record<MusicTrack, string> = {
  ...
  boss: '/music/boss-theme.mp3',
  defeat: '/music/defeat.mp3',
};

const SFX_PATHS: Record<SFXType, string> = {
  'turret-fire': '/sfx/turret-fire.mp3',
  'boss-appear': '/sfx/boss-appear.mp3',
  'boss-death': '/sfx/boss-death.mp3',
  'player-hurt': '/sfx/player-hurt.mp3',
  'player-death': '/sfx/player-death.mp3',
  'enemy-death': '/sfx/enemy-death.mp3',
...
// Music volume reduced by 20% (0.5 -> 0.4)
// Turret fire volume reduced by 30% (0.4 -> 0.28)
const SFX_DEFAULTS: Record<SFXType, SFXOptions> = {
  'turret-fire': { throttleMs: 100, pitchVariation: 0.25, volume: 0.28 },
  'boss-appear': { throttleMs: 0, pitchVariation: 0, volume: 0.8 },
  'boss-death': { throttleMs: 0, pitchVariation: 0, volume: 0.9 },
  'player-hurt': { throttleMs: 200, pitchVariation: 0.1, volume: 0.5 },
  'player-death': { throttleMs: 0, pitchVariation: 0, volume: 0.8 },
  'enemy-death': { throttleMs: 80, pitchVariation: 0.2, volume: 0.3 },
  'levelup': { throttleMs: 0, pitchVariation: 0, volume: 0.6 },
...
class AudioManagerClass {
  ...
  private musicVolume: number = 0.3; // Reduced by 20% from 0.5
  ...
  toggleMute(): boolean {
    ...
    if (this.currentMusic) {
      this.currentMusic.volume = this.isMuted ? 0 : this.getEffectiveMusicVolume();
    }
    return this.isMuted;
  }
