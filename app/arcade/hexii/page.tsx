...
const Game = dynamic(
  ...
  {
    ssr: false,
    loading: () => <div className="w-screen min-h-screen flex justify-center items-start overflow-y-auto overflow-x-hidden font-orbitron">Loading game...</div>,
  }
);
  ...
  return (
    <div className="relative w-full min-h-screen bg-[radial-gradient(ellipse_at_30%_20%,rgba(55,66,250,0.15)_0%,transparent_50%),radial-gradient(ellipse_at_70%_80%,rgba(255,71,87,0.1)_0%,transparent_50%),linear-gradient(180deg,#0a0a14_0%,#1a1a2e_50%,#0a0a14_100%)] flex justify-center items-start overflow-visible">
      <div className="absolute top-5 left-5 right-5 z-20 flex justify-between items-center">
        <Link href="/arcade" className="font-orbitron inline-block text-white/60 no-underline text-sm tracking-wider transition-colors hover:text-white/90">
          ← Back to Arcade
        </Link>
      ...
      <div className="relative z-10 text-center px-4 md:px-10 pt-24 md:pt-32 pb-10 w-full max-w-[1200px]">
        <h1 className="font-orbitron text-4xl md:text-6xl lg:text-8xl xl:text-9xl font-black tracking-[10px] md:tracking-[15px] lg:tracking-[20px] mb-2 bg-gradient-to-r from-white via-[#a8a8ff] to-white bg-clip-text text-transparent drop-shadow-[0_0_40px_rgba(100,100,255,0.5)]">
          <span className="inline-block mx-2 md:mx-5 animate-[rotate_10s_linear_infinite]">⬡</span>
          HEXII
          <span className="inline-block mx-2 md:mx-5 animate-[rotate_10s_linear_infinite]">⬡</span>
        </h1>
        ...
        {personalBest !== null && (
          <div className="font-orbitron text-xs md:text-sm text-white/50 tracking-wide mb-4 flex items-center justify-center gap-2">
            Your Best: <span className="text-[#ffa502] font-bold text-sm md:text-base">{personalBest.toLocaleString()}</span>
          </div>
        )}
        
        <div className="mb-8 md:mb-12">
          <h2 className="font-orbitron text-xs md:text-sm tracking-[3px] md:tracking-[6px] text-white/50 mb-6">SELECT YOUR CORE</h2>
          <div className="flex flex-wrap justify-center gap-4 md:gap-4">
            {colors.map(({ color, name, desc }) => {
              const glowColors: Record<HexColor, string> = {
                ...
                GREEN: 'drop-shadow-[0_0_20px_rgba(46,213,115,0.8),0_0_40px_rgba(46,213,115,0.4)]',
                YELLOW: 'drop-shadow-[0_0_20px_rgba(255,165,2,0.8),0_0_40px_rgba(255,165,2,0.4)]',
                ...
                ORANGE: 'drop-shadow-[0_0_20px_rgba(255,99,72,0.8),0_0_40px_rgba(255,99,72,0.4)]',
              };
              return (
                <button
                  key={color}
                ...
                >
                  <div className={`text-3xl md:text-5xl mb-3 ${glowColors[color]}`}>⬢</div>
                  <div className="font-orbitron text-xs md:text-sm font-bold tracking-wide mb-2">{name}</div>
                  <div className="font-orbitron text-[10px] text-white/50 tracking-wide">{desc}</div>
                </button>
              );
            })}
          </div>
        ...
        <div className="font-orbitron mt-8 md:mt-10 text-xs text-white/40 tracking-wide">
          <p><strong className="text-white/70">WASD</strong> to move • <strong className="text-white/70">MOUSE</strong> to aim</p>
        </div>
